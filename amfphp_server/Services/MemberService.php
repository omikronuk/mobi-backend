 ...
 */
class MemberService extends ActiveRecord\Model  {
	static $table = 'kw_members';

    
	
	function createMember($data=array()){
	...
	function getMember($id){
        ...
        function updateMember($id, $data=array()){
             $isMem = self::getMember($id);
             if(!empty($isMem)):
                $rs = self::table()->update($data, array('id' => $id));
	...
	function login($email, $pword){
            ...
            if(self::validateEmail($email)):
                ...
                $rs = self::isMemberActive($email, $pword);
       ...
       function isEmailTaken($email){
        ...
        function isMemberFound($username, $password){
                 $pword = md5($password);
		return (bool) self::getMemberID($username, $password, '`active`=1 ');
	}
         
       function isMemberActive($email, $pword){
                $pword = md5($pword);
         ...
         function getMemberID($email, $pword){
